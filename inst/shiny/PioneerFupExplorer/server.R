...
shinyServer(function(input, output, session) {
  ...
  typeSankeyData <- reactive({
    target_id <- andrData$cohort_count %>%
      dplyr::filter(databaseId %in% !!input$databasesTreatmentTypePatterns, cohortId %in% !!cohortIdTreatmentTypePatterns()) %>%
      dplyr::pull(cohortId)
    
    sankeyData <- andrData$treatment_type_pathways %>%
        dplyr::filter(sourceId == target_id, databaseId == !!input$databasesTreatmentTypePatterns) %>%
        dplyr::collect()
      return(sankeyData)
  })

  # Sankey
  sendTypeSankeyData <- function(){
    data <- typeSankeyData()
    jsonData <- jsonlite::toJSON(data, pretty = TRUE)
    session$sendCustomMessage(type = "jsondata_type", jsonData)
  }

  observeEvent(input$databasesTreatmentTypePatterns, {
    sendTypeSankeyData()
  })

  observeEvent(input$strataTreatmentTypePatterns, {
    sendTypeSankeyData()
  })
  
  
  output$typeSankeyData <- renderDataTable({
    data <- typeSankeyData()
    if(nrow(data > 30)){
      DT::datatable(
        data %>%
  ...
  # Treatment Type Time To Event ---------------------
  
  output$timeToEventHeader <- renderText({
    paste('Time to ', input$TypeTreatmentTypePatterns)
  })
  
  output$timeToTreatmentTypePlot <- renderPlot({
    target_id <- andrData$cohort_count %>%
      dplyr::filter(databaseId %in% !!input$databasesTreatmentTypePatterns, cohortId %in% !!cohortIdTreatmentTypePatterns()) %>%
      dplyr::pull(cohortId)
    ...
    timeToTreatmentType <- timeToTreatmentType %>%
      dplyr::select(time, surv, n.censor, n.event, n.risk, upper, lower) 
    timeToTreatmentType$strata <- paste('Time To ', eventName)
    
    color_map = c(' ' = "#838383")
    ggsurvplot_core(timeToTreatmentType,
                    risk.table = "nrisk_cumcensor",
                    palette = color_map,
                    legend.labs = ' ',
                    cmap = color_map,
                    ...
                    surv.median.line = 'hv',
                    legend.title = ' ',
                    ylim = c(min(timeToTreatmentType$lower), 1),
                    ggtheme = ggplot2::theme_bw()
    )
  })
