...
FROM (
  ...
  FROM (SELECT concepts.diag_category_id, score.weight, cohort.subject_id, cohort.cohort_definition_id
  	FROM 
  	@cohort_database_schema.@cohort_staging_table cohort
  	...
  	RIGHT JOIN @cohort_database_schema.@cohort_staging_table c
  	...
  	SELECT 
          ...
          cohort.subject_id, 
          cohort_start_date
  	FROM @cohort_database_schema.@cohort_staging_table cohort
...
FROM (
  ...
  INNER JOIN @cohort_database_schema.charlson_map cm 
    ON c.subject_id = cm.subject_id
    AND c.cohort_definition_id = cm.cohort_definition_id
    AND (
      cm.weight @lb_operator @lb_strata_value
    )
) s
