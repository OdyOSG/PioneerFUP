generateTreatmentTypePathwaysInfo <- function(connection, cohortDatabaseSchema, cohortTable, databaseId) {
  ...
  treatmentCohortsIds <- sapply(pathwaysCohorts$treatmentCohorts, function(x) x$id)
  ...
  result %>% 
    ...
    tidyr::replace_na(list(atlasName = "No following treatment cohort")) %>% 
